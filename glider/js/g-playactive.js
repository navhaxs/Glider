...
function Collision(whatKind, who) {
  ...
  switch (whatKind) {
    ...
    case crashIt:
      ...
      if (theGlider.isForward) theGlider.srcNum = 0;
      else theGlider.srcNum = 2;
      break;

    case liftIt:
      if (!airOut) liftAmount = -7;
      break;
    case dropIt:
      if (!airOut) liftAmount = 7;
      break;
    ...
    case burnIt:
      ...
      if (lifeNormal && theGlider.destRect.bottom > eventKind[who][1]) {
        DoTheSound(8);
        lifeNormal = false;
        if (theGlider.isRight) theGlider.phase = 1;
      ...
      if (eventKind[who][2] < TickCount()) {
        ...
        switch (o.objectIs) {
          case lftFan:
            ...
            } else eventKind[tempInt][0] = ignoreIt;
            break;
          case ritFan:
            if (eventKind[tempInt][0] == ignoreIt) {
              DoTheSound(3);
...
function SearchRects() {
  ...
  if (theGlider.destRect.bottom > kFloorLimit) {
    // check floor
    BiteIt();
    return;
  }
...
function HandleBand() {
  if (theBand.whole.left > 512 || theBand.whole.left < -16) bandBorne = false;

  theBand.phase++;
  if (theBand.phase > 2) theBand.phase = 0;
  theBand.whole = CopyRect(theBand.dest);
  theBand.dest.left += theBand.velocity;
  theBand.dest.right += theBand.velocity;
  if (theBand.velocity > 0) theBand.whole.right += theBand.velocity;
  else theBand.whole.left += theBand.velocity;
...
function GetInput() {
  if (controlMethod == holdKey) {
    ...
    if (
      ...
      theGlider.forVel = kMaxThrust;
    else theGlider.forVel = -kMaxThrust;
    if (theKeys[leftKey]) {
      if (theGlider.isRight) {
        begin;
        theGlider.isForward = false;
    ...
    if (a.unSeen) {
      ...
      switch (a.kind) {
        ...
        case 2:
          a.destRect.top += a.vertOff;
          a.destRect.bottom += a.vertOff;
          if (a.vertOff > 0) a.wholeRect.bottom += a.vertOff;
          else a.wholeRect.top += a.vertOff;
          break;
      }

      if (!a.unseen && !RectLap(a.wholeRect, wholeArea)) {
...
function MoveCycleObs() {
  ...
  for (let index = 1; index <= nCycleObs; index++) {
    ...
    switch (co.kindIs) {
      case candle:
        co.position++;
        if (co.position > 50) co.position = 48;
        break;
      ...
      case grease:
        if (eventKind[co.tiedTo][0] == ignoreIt) {
          ...
          if (co.reset > 4) {
            ...
            if (co.velocity > co.position) co.reset = 999;
          } else {
            if (co.reset < 3) co.accel = 58;
            else co.accel = 59;
          }
        }
        break;
      ...
      case drip:
        if (co.phase < 57) {
          if (binaryFlip != 0) {
            co.phase++;
            if (co.phase == 57) co.position = co.position + 160;
          }
        } else {
      ...
      case toastr:
        if (co.phase != 0) {
          ...
          if (co.position > co.reset) {
            ...
            co.reset =
              TickCount() +
              thisHouse.theRooms[roomAt].theObjects[co.tiedTo].extra;
            co.phase = 0;
          }
          co.wholeRect = CopyRect(eventRect[co.tiedTo]);
          let er = eventRect[co.tiedTo];
          ...
          er.bottom += diff;
          er.top += diff;
          if (diff > 0) co.wholeRect.bottom += diff;
          else co.wholeRect.top += diff;
        } else {
      ...
      case fshBwl:
        if (co.phase != 0) {
          if (co.velocity > -16 && co.velocity < 16) co.phase = 69;
          else {
            if (co.velocity < 0) co.phase = 66;
            else co.phase = 68;
          }
          co.velocity += co.accel;
          co.position += co.velocity;
...
function MonitorStorm() {
  if (theLightning.whatPhase == 0) {
    if (TickCount() > theLightning.whatTime) theLightning.whatPhase = 1;
...
function DrawCScene() {
  ...
  if (hasWindow && theLightning.whatPhase != 0) {
    ...
    context.clip();
    PenNormal();
    if (theLightning.whatPhase == 4) context.fill();
    else {
      context.beginPath();
      context.lineWidth = 2;
      ...
      MoveTo(
        theLightning.theBolts[bolt][0][0],
        theLightning.theBolts[bolt][0][1]
      );
      for (let index = 1; index < 8; index++) {
        LineTo(
          theLightning.theBolts[bolt][index][0],
          theLightning.theBolts[bolt][index][1]
        );
        //Move(0, 2);
      }
      context.stroke();
    ...
    for (let index = 1; index <= nCycleObs; index++) {
      ...
      switch (co.kindIs) {
        ...
        case fshBwl:
          ...
          else DrawMaskedObject(srcRect[69], srcRect[69], eventRect[co.tiedTo]);
          break;
      }
  ...
  else DrawMaskedObject(shadoRct[1], shadoRct[1], theGlider.shadoDest);
  if (lifeNormal)
    DrawMaskedObject(
      glideRct[theGlider.srcNum],
      glideRct[theGlider.srcNum],
    ...
    switch (theGlider.mode) {
      ...
      case fadingOut:
        ...
        if (!theGlider.isRight) tempInt = tempInt + 7;
        DrawMaskedObject(
          glideRct[srcNum],
          glideRct[tempInt],
          theGlider.destRect
      ...
      case shredding:
        let tempRect = SetRect(
          256,
          208 - (theGlider.destRect.bottom - theGlider.destRect.top),
          304,
          208
        );
...
function DoSpecial() {
  switch (theGlider.mode) {
    case fadingIn:
      if (theGlider.phase == 1) DoTheSound(2);
      theGlider.phase++;
      if (theGlider.phase > 16) {
        if (theGlider.isRight) theGlider.srcNum = 0;
        else theGlider.srcNum = 2;
        lifeNormal = true;
        theGlider.phase = 0;
    ...
    case fadingOut:
      ...
      if (theGlider.phase > 16) OffAMortal();
      break;
    case turnRt2Lf:
      theGlider.phase++;
...
function Coordinate() {
  ...
  if (isScrolling) {
    if (thisTickCount >= scrollingNextTime) isScrolling = ScrollStep();
    if (!isScrolling) pausing = false;
    return;
  }

  if (inClosingAnimation) {
    if (thisTickCount >= closingAnimationNextTime) ClosingAnimationStep();
    return;
  }

  if (gameOver) {
    if (thisTickCount >= gameOverNextTime) GameOverStep();
    return;
  }

  if (Math.round(thisTickCount - LastTickCount) < 2) return;
  LastTickCount = thisTickCount;
  if (playing && !pausing) {
    timeIs = thisTickCount;
    liftAmount = 2;
    binaryFlip ^= 1;
    if (bandBorne) HandleBand();
    if (lifeNormal) {
      SearchRects();
    ...
    if (playing) {
      if (nAnimates > 0) MoveAnimates();
      if (nCycleObs > 0) MoveCycleObs();
      if (hasWindow) MonitorStorm();

      DrawCScene();

      if (scoreIsRolling) BumpTheScore();
    }
    ...
    if (bassLoop >= playBassTime) {
      ...
      playBassTime =
        ...
        kMinBassLoop;
      if (playBassTime < kMinBassLoop) playBassTime = kMinBassLoop;
    }
  }
}
