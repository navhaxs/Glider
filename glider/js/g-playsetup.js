...
function CheckForTimeBonus() {
  ...
  if (loopsThruRoom < kBonusTimeToBeat)
    timeBonus =
      (kBonusTimeToBeat - loopsThruRoom) * (Math.floor(roomAt / 5) + 1);
...
function SetRoomState() {
  ...
  if (didntExitEntrance) roomVisits[roomAt] = true;
  ...
  for (let [i, o] of r.theObjects.entries()) {
    ...
    switch (eventKind[i][0]) {
      ...
      case lightIt:
      case airOnIt:
        if ((!lightsOut || forceLightsOn) && !airOut) r.conditionCode = 0;
        break;
    }
  }
}
...
function ScrollForwardStep() {
  ...
  if (scrollingIteration == 32) {
    ReadyRoom();
    return false;
  } else scrollingNextTime += scrollingTicks;
  return true;
}
...
function ScrollUp() {
  ...
  for (let o of thisHouse.theRooms[roomAt].theObjects) {
    if (o == null) continue;
    if (o.objectIs == dnStar) leftCorner = o.boundRect.left + 64;
  }
...
function ScrollDown() {
  ...
  for (let o of thisHouse.theRooms[roomAt].theObjects) {
    if (o == null) continue;
    if (o.objectIs == upStar) leftCorner = o.boundRect.left + 64;
  }
...
function Transport(whatRoom) {
  ...
  if (roomAt != 0) {
    ...
    for (let [index, o] of thisHouse.theRooms[roomAt].theObjects.entries()) {
      if (o == null) continue;
      if (o.objectIs == celDct && o.isOn) leftCorner = o.boundRect.left;
    }
...
function CastBolt() {
  ...
  for (let i = 0; i <= boltLength; i++) {
    ...
    if (start <= boltRect.left) boltRect.left = start;
    if (finish <= boltRect.left) boltRect.left = finish;
    if (start >= boltRect.right) boltRect.right = start;
    if (finish >= boltRect.right) boltRect.right = finish;
...
function ClosingAnimationStep() {
  ...
  if (closingAnimationIteration == 200) inClosingAnimation = false;
}
