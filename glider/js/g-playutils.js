...
function ResetGlider() {
  ...
  if (g.mode != burning) {
    ...
    g.phase = 0;
    g.isForward = true;
    if (g.isRight) g.srcNum = 0;
    else g.srcNum = 2;
  }
  if (enteredLeft) g.destRect = SetRect(0, 40, 48, 60);
  else g.destRect = SetRect(464, 40, 512, 60);
...
function DrawTable(o) {
  ...
  tempRect = OffsetRect(
    tempRect,
    Math.floor((theRect.top - kFloorVert) / 5),
    0
  );
  ...
  context.beginPath();
  MoveTo(
    Math.floor((theRect.left + theRect.right) / 2) + 2,
    theRect.bottom + (theRect.right - theRect.left) / 8
  );
  ...
  context.beginPath();
  MoveTo(
    Math.floor((theRect.left + theRect.right) / 2),
    theRect.bottom + (theRect.right - theRect.left) / 8
  );
  LineTo(Math.floor((theRect.left + theRect.right) / 2), kFloorVert - 7);
  context.stroke();
  ...
  tempRect = OffsetRect(
    tempRect,
    Math.floor((theRect.left + theRect.right) / 2) - 31.5,
    kFloorVert - 11
  );
...
*/
  ...
  if (panels == 0) {
    ...
    let width = Math.floor(
      (theRect.right - theRect.left - (panels + 1) * 5) / panels
    );
    tempR = SetRect(
      theRect.left + 5,
      theRect.top + 5 + offIt,
      theRect.left + 5 + width,
      theRect.bottom - 5 - offIt
    );
...
function DrawWindow(o) {
  ...
  if (windowOpen) {
    tempRect = OffsetRect(
      tempRect,
      0,
      26 - Math.floor((theRect.bottom - theRect.top) / 2)
    );
  }
  FillNFrame(rgbBrown, tempRect);
  tempRect = InsetRect(tempRect, 6, 6);
...
function DrawStair(o) {
  ...
  if (kind == upStar) img = artFiles.get(198);
  else img = artFiles.get(199);
