...
function ReadyTea(o, index) {
  ...
  if (eventRect[index].top < kCeilingVert) eventRect[index].top = kCeilingVert;
  if (eventRect[index].left < 0) eventRect[index].left = 0;
...
function FrameWindow(o, index) {
  ...
  for (let i = 1; i <= 3; i++) {
    l.theBolts[i][0][0] =
      tempRect.left + Randomize(tempRect.right - tempRect.left);
    l.theBolts[i][0][1] = tempRect.top;
    for (let joint = 1; joint <= 7; joint++) {
      do {
        l.theBolts[i][joint][0] =
          l.theBolts[i][joint - 1][0] + Randomize(tempInt * 4) - tempInt * 2;
        l.theBolts[i][joint][1] = tempInt * joint + tempRect.top;
      } while (
        l.theBolts[i][joint][0] <= tempRect.left ||
        l.theBolts[i][joint][0] >= tempRect.right
      );
    }
  }
  ...
  for (let [index, o] of r.theObjects.entries()) {
    ...
    switch (o.objectIs) {
      ...
      case celVnt:
        ...
        break;
      case celDct:
        if (o.isOn) eventKind[index][0] = dropIt;
        else {
          eventKind[index][0] = moveIt;
      ...
      case candle:
        ...
        break;
      case lftFan:
        if (o.isOn) eventKind[index][0] = turnItLeft;
        else eventKind[index][0] = ignoreIt;
        break;
      case ritFan:
        if (o.isOn) eventKind[index][0] = turnItRight;
        else eventKind[index][0] = ignoreIt;
        break;
      case clock:
        eventKind[index][0] = awardIt;
        eventKind[index][1] = o.amount;
        if (o.amount < 1) eventRect[index] = CopyRect(nullRect);
        break;
      case paper:
        ...
        break;
      case grease:
        if (o.isOn) eventKind[index][0] = spillIt;
        else eventKind[index][0] = slideIt;
        break;
      case bnsRct:
      ...
      case thermo:
        ...
        break;
      case shredr:
        if (o.isOn) eventKind[index][0] = shredIt;
        else eventKind[index][0] = ignoreIt;
        break;
      case pwrSwt:
